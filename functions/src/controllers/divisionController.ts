...
export async function getDivisionRules(req: Request, res: Response) {
  const userId = req.user?.userId;

  try {
    const result = await pool.query(
      `SELECT * FROM rules_division 
       WHERE casal_id = $1 
...
export async function createDivisionRule(req: Request, res: Response) {
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO rules_division (casal_id, tipo, parametros, categoria, ativa)
       VALUES ($1, $2, $3, $4, true)
       RETURNING *`,
      [userId, tipo, JSON.stringify(parametros), categoria || null]
    );
...
export async function updateDivisionRule(req: Request, res: Response) {
  const { id } = req.params;
  ...
  try {
    const result = await pool.query(
      `UPDATE rules_division 
       ...
       RETURNING *`,
      [tipo, parametros ? JSON.stringify(parametros) : null, categoria, ativa, id, userId]
    );
...
export async function deleteDivisionRule(req: Request, res: Response) {
  const { id } = req.params;
  const userId = req.user?.userId;

  try {
    await pool.query(
      `DELETE FROM rules_division WHERE id = $1 AND casal_id = $2`,
      [id, userId]
    );
...
export async function calculateDivision(req: Request, res: Response) {
  ...
  try {
    ...
    const ruleResult = await pool.query(query, params);
    ...
    switch (rule.tipo) {
      ...
      case 'proporcional':
        ...
        const rendaResult = await pool.query(
          `SELECT p.renda_mensal FROM profiles p
